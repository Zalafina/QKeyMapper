...
{
    Q_UNUSED(extraInfo);  // Mouse wheel events are not tracked in hook for now
...
#ifdef DEBUG_LOGOUT_ON
        if (mouseQueueSize > 0) {
            qDebug("[clearFakerInputExtraInfoQueues] Cleared mouse ExtraInfo queue, was size: %d", mouseQueueSize);
        }
#else
