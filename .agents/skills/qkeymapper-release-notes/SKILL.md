...
- 默认沿用现有产品版本，Build 日期使用用户当前时区的当天日期；用户指定值优先。不修改程序版本、标签或发布配置。
...
- 若内容已经完整、没有实质更新，不为日期变化而改动 README，不创建空条目或空 commit。需要实质更新时采用本次确认的版本日期，并保持日期降序；日期冲突或未来日期不明确时请用户确认。
- 沿用 `* vX.Y.Z(Build YYYYMMDD)` 和缩进的 `*` 条目格式。每条通常一两句话，优先写操作、效果、必要限制；相近功能合并。快捷键注明键名及必要的焦点/状态条件，避免暴露函数、宏、线程、提交哈希等实现细节。不要把“取消程序内权限拦截”夸大为“绕过系统权限限制”。
