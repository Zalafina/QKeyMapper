...
---------------
### 新添加功能列表(根据更新时间降序排列)
* v1.3.7(Build 20241118)
    * 映射项设定窗口中添加"检查组合键按下顺序"勾选框，勾选后如果原始按键是组合键则检查组合键触发时按下的按键顺序是否与原始按键排列顺序一致，一致时才触发组合键。例如："A+B"的原始按键组合键勾选"检查组合键按下顺序"后，先按下A再按下B才会触发组合键发送映射按键。
