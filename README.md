...
## 📖 软件简介
## QKeyMapper是兼容Win7/Win10/Win11的开源按键映射工具，使用Qt Widget 和 WinAPI 基于C++语言开发。可支持键盘、鼠标、游戏手柄、虚拟游戏手柄之间各种互相按键映射。软件下载解压后直接运行，不修改注册表、不需重新启动系统，映射功能可立即生效和停止。适用于各种游戏或办公等目的的Windows PC端改键场景。
...
---------------
### 🎯 新添加功能列表(根据更新时间降序排列)
* v1.3.8(Build 20250722)
    * 设定名称规则变化，添加"设定名称"单行编辑框，用户可以自定义编辑设定名称，列表中不存在的设定名称会保存时会添加新的设定。点击保存时，设定名称如果与列表中已存在的名称相同会覆盖保存设定。
    ...
    * "通用设定"标签页添加"启动时自动开始"勾选框，勾选后软件启动时启动进入"映射开始"状态。不再需要使用"自动匹配前台进程"勾选框来达到启动后自动"映射开始"的效果。
