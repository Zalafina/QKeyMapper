...
---------------
### 🎯 新添加功能列表(根据更新时间降序排列)
* v1.3.8(Build 20260909)
    * 映射表操作新增快捷键：**Ctrl+N** 添加空白Tab，**Ctrl+D** 复制当前Tab，菜单中同步显示快捷键。仅在主窗口有焦点且未开始映射时生效，按键捕获时不会触发，支持连续按下操作。
    * 添加空白Tab后自动切换到新Tab；复制当前Tab后仍停留原Tab，方便连续复制。快捷键、右键菜单和顶部菜单操作效果一致。
    * “保存设定”快捷键统一为 **Ctrl+S**，支持键盘左右两侧Ctrl键，快捷键提示同步更新。
