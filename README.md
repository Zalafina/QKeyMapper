...
* v1.3.6(Build 20240221)
    ...
    * 更改组合键映射策略，现在"原始组合键"改为文本框来手动输入，现在可以支持不包含Ctrl、Alt、Shift、Win的组合键映射。
    * 删除"禁用Win键"勾选框，现在可以通过映射按键中选择"BLOCKED"来禁用任意按键。
    ※注意: v1.3.6(Build 20240221)对部分按键名称进行了更改，可能出现无法加载旧版本配置文件的情况，建议更新此版本前备份保存可执行文件路径下的 keymapdata.ini 配置文件。
    ※可以根据新Release版本ZIP压缩包中的 CombinationSupportKeys.txt 文件中的按键名称编辑修改 keymapdata.ini 配置文件中原来的按键名称来继续使用旧配置文件。
