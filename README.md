...
* v1.3.7(Build 20240622)
    * 映射列表中添加"WindowPoint"映射,可以使用L-Alt+鼠标左键来标记当前映射设定对应窗口的窗口内相对坐标，选择Mouse-L|R|M|X1|X2_WindowPoint可以发送窗口内的相对鼠标坐标点击。"F8"按键显示当前设定映射窗口内的WindowPoint坐标点。窗口内坐标点颜色与之前添加的"F9"按键的全屏坐标点显示颜色相同。    
