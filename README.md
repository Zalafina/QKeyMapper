...
* v1.3.8(Build 20260926)
    * 修复在 Qt5 等环境下配置文件(INI)中非 ASCII 字符(如中文设定名称、托盘图标路径等)在保存设定或窗口映射切换后被异常改写为 Unicode 转义字符(\xXXXX)的问题，确保统一以原生 UTF-8 编码存储。
* v1.3.8(Build 20260920)
    * 修复映射表中“Block-Input”等相关按键映射无法勾选连发与锁定的问题。
    * 优化“映射表操作”菜单中快捷键的对齐与间距显示。
    * 修复后台崩溃诊断模块启动时鼠标指针短暂显示忙碌光标的问题。
    * 修复在“映射项设定”中更新映射后主表高亮选区与焦点丢失的问题，更新时保持视口滚动位置。
    * 优化映射表删除映射项后的重新选中逻辑，自动跳过隐藏行与分隔行并定位至邻近可见行。
    * 新增“坐标拾取窗口”独立悬浮窗(通过顶部"视图"菜单选项开启)，支持屏幕与目标窗口物理坐标实时显示及准星拖拽取点，并在运行期间记忆悬浮窗拖拽位置。
    * 优化深色主题下输入与数值控件边框显示。
* v1.3.8(Build 20260912)
    * 修复视图选项切换及设定变更提示相关问题。
    * 映射表操作新增快捷键：**Ctrl+N** 添加空白Tab，**Ctrl+D** 复制当前Tab。仅在主窗口有焦点、未开始映射且未捕获按键时生效；添加后自动切换到新Tab，复制后仍停留原Tab。
    * “保存设定”快捷键统一为 **Ctrl+S**，支持键盘左右两侧Ctrl键。
    * 取消启动时新增的管理员权限不足弹窗拦截，改善部分限制管理员权限的电脑上的启动兼容性。
    * 修复几处启动或运行中可能导致QKeyMapper崩溃的问题。
    * 新增崩溃及疑似卡死时自动保存诊断文件的功能，文件默认保存在 **log** 子目录，便于定位问题。
...
##### 坐标点选取方式：
- **推荐方式**：从顶部菜单栏「视图」→ 勾选「显示坐标采集悬浮窗」，打开独立的坐标采集置顶小窗。可切换「屏幕」或「窗口」模式，实时预览鼠标所在物理坐标，按住瞄准图标拖动到目标位置后松开鼠标左键即可完成坐标采集并自动同步到主窗口；拖拽期间点击鼠标右键可取消采集。
- **快捷键方式**：亦可继续使用全局快捷键进行快速采集：
  - WindowPoint 映射键：在目标窗口处于匹配激活状态下，使用「左 Alt 键 + 鼠标左键」点击窗口内目标位置采集相对坐标；
  - ScreenPoint 映射键：使用「左 Ctrl 键 + 鼠标左键」点击屏幕任意位置采集全屏物理坐标。
