...
* v1.3.7(Build 20241210)
    * 为下面虚拟手柄按键添加"轻推值"设定滑动条，"轻推值"范围1~254，默认滑动条在最大位置，滑动条为最大值255时，不添加[xxx]的后缀数字，与以前版本的手柄按键按到底的最大值相同。添加映射键效果示例：vJoy-LS-Up[150]、vJoy-Key12(RT)[100]
        * 支持按键列表：vJoy-Key11(LT)/vJoy-Key12(RT)/vJoy-LS-Up/vJoy-LS-Down/vJoy-LS-Left/vJoy-LS-Right/vJoy-RS-Up/vJoy-RS-Down/vJoy-RS-Left/vJoy-RS-Right
