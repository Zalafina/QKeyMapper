...
* v1.3.7(Build 20241116)
    * 映射项设定窗口中添加"录制按键"按钮，按下此按钮弹出按键录制对话框。录制对话框下按下"F11"键开始进行按键录制，按下"F12"键停止按键录制，也可以鼠标左键点击"开始录制"/"停止录制"按钮来开始和停止按键录制。按键录制结束后的按键记录显示在编辑框中，并且自动复制到了剪贴板。之后可以直接将按键记录内容粘贴到"映射按键"编辑框等位置。
