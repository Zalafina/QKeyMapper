...
* v1.3.7(Build 20241130)
    * 映射项设定窗口中添加"不可打断"勾选框，勾选后同一个原始按键的映射按键未发送完毕之前再次按下相同的原始按键不打断正在发送的映射按键。未勾选状态下默认重复按下相同原始按键会打断当前正在发送中的映射按键。
    * 映射项设定窗口中编辑映射按键时对后覆盖"！"前缀进行检查。后覆盖前缀"！"不能使用在按键序列中。相同的映射按键不能与后覆盖前缀"！"同时使用，例如: "A+！A"会检查为映射按键冲突错误。
